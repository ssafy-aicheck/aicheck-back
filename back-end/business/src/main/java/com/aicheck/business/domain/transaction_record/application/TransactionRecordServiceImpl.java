...
public class TransactionRecordServiceImpl implements TransactionRecordService {
	...
	public CalendarRecordListResponse getCalendarData(Long memberId, int year, int month) {
		...
		for (TransactionRecord record : records) {
			long amount = switch (record.getType()) {
				case DEPOSIT, INBOUND_TRANSFER -> record.getAmount();
				case PAYMENT, WITHDRAW, OUTBOUND_TRANSFER -> -record.getAmount();
			};

			LocalDate date = record.getCreatedAt().toLocalDate();
			sumByDate.put(date, sumByDate.getOrDefault(date, 0L) + amount);
	...
	public void updateTransactionRecord(UpdateTransactionRecordRequest request) {
		...
		FirstCategory firstCategory = record.getFirstCategory();
		SecondCategory secondCategory = record.getSecondCategory();
		String description = record.getDescription();

		if (request.getFirstCategoryId() != null && request.getSecondCategoryId() != null) {
			firstCategory = firstCategoryRepository.findById(request.getFirstCategoryId())
				.orElseThrow(() -> new BusinessException(BusinessErrorCodes.FIRST_CATEGORY_NOT_FOUND));

			secondCategory = secondCategoryRepository
		...
		record.updateCategoryAndDescription(firstCategory, secondCategory, description);
	}
	...
	public DescriptionRatioResponse getDescriptionRatio(final Long memberId, final YearMonth targetMonth) {
		...
		Object[] result = (Object[])transactionRecordRepository.countTotalAndDescribed(memberId, startOfMonth,
			endOfMonth);
